...
bool MoveValidator::isBlockedBySameColor(const Position& to, const Board& board, const Piece* piece) {
	Piece* targetPiece = board.getPieceAt(to);
	return targetPiece != nullptr && targetPiece->getColor() == piece->getColor();
}

int MoveValidator::calculateDuration(const Position& source, const Position& target) {
